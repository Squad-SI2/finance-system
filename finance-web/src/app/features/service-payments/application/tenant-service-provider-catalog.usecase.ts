...
export interface TenantServiceProviderCatalogState {
...
export class TenantServiceProviderCatalogUseCase {
  private readonly servicePaymentsService = inject(ServicePaymentsService);

  private readonly state = signal<TenantServiceProviderCatalogState>({
    status: 'idle',
    catalog: [],
    error: null
  });
  ...
  async loadCatalog(): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(this.servicePaymentsService.listTenantServiceProviderCatalog());
