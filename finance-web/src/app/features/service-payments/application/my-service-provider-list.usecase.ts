...
export interface MyServiceProviderListState {
...
export class MyServiceProviderListUseCase {
  private readonly servicePaymentsService = inject(ServicePaymentsService);

  private readonly state = signal<MyServiceProviderListState>({
    status: 'idle',
    page: null,
    error: null,
    filter: {}
  ...
  async loadProviders(
    ...
    try {
      const response = await firstValueFrom(
        this.servicePaymentsService.listMyServiceProviders(page, size, filter)
      );
      ...
      this.state.set({
        status: 'error',
        page: null,
        error: response.message || 'No se pudieron cargar los proveedores disponibles',
        filter
      });
    } catch (err: any) {
      this.state.set({
