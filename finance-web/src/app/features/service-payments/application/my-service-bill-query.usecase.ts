...
export interface MyServiceBillQueryState {
...
export class MyServiceBillQueryUseCase {
  private readonly servicePaymentsService = inject(ServicePaymentsService);

  private readonly state = signal<MyServiceBillQueryState>({
    status: 'idle',
    result: null,
    selectedBill: null,
    error: null
  ...
  async queryBills(request: QueryServiceBillsRequest): Promise<QueryServiceBillsResponse> {
    ...
    try {
      const response = await firstValueFrom(this.servicePaymentsService.queryMyServiceBills(request));
