...
export interface PlatformServiceProviderCatalogState {
...
export class PlatformServiceProviderCatalogUseCase {
  private readonly servicePaymentsService = inject(ServicePaymentsService);

  private readonly state = signal<PlatformServiceProviderCatalogState>({
    status: 'idle',
    catalog: [],
    error: null
  });
  ...
  async loadCatalog(): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(this.servicePaymentsService.listPlatformProviderCatalog());
      ...
      this.state.set({
        status: 'error',
        catalog: [],
        error: response.message || 'No se pudo cargar el catálogo de códigos'
      });
    } catch (err: any) {
      this.state.set({
        status: 'error',
