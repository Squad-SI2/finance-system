...
CREATE TABLE IF NOT EXISTS public.backup_jobs (
    ...
    CONSTRAINT chk_backup_jobs_operation_type CHECK (operation_type IN ('BACKUP', 'RESTORE')),
    CONSTRAINT chk_backup_jobs_scope CHECK (scope IN ('TENANT_SCHEMA', 'FULL_DATABASE')),
    CONSTRAINT chk_backup_jobs_status CHECK (status IN ('PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'RESTORING', 'RESTORED', 'RESTORE_FAILED')),
    CONSTRAINT chk_backup_jobs_format CHECK (format IN ('CUSTOM', 'SQL'))
);

CREATE INDEX IF NOT EXISTS idx_backup_jobs_tenant_slug ON public.backup_jobs(tenant_slug);
...
CREATE INDEX IF NOT EXISTS idx_backup_jobs_scope ON public.backup_jobs(scope);

CREATE UNIQUE INDEX IF NOT EXISTS uq_backup_jobs_active_per_tenant
    ON public.backup_jobs(tenant_slug)
    WHERE tenant_slug IS NOT NULL AND status IN ('PENDING', 'RUNNING', 'RESTORING');

CREATE UNIQUE INDEX IF NOT EXISTS uq_backup_jobs_active_full_database
    ON public.backup_jobs(scope)
    WHERE scope = 'FULL_DATABASE' AND status IN ('PENDING', 'RUNNING', 'RESTORING');
