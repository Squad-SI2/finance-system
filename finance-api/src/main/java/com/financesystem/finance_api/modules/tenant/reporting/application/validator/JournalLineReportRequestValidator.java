...
@Component
public class JournalLineReportRequestValidator {

    private final ReportTemplate template;

    public JournalLineReportRequestValidator(JournalLinesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticJournalLinesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.JOURNAL_LINES);
    ...
    public void validateManagerial(RunManagerialJournalLinesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.JOURNAL_LINES);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<JournalLineReportColumnKey> columns) {
        ...
        for (JournalLineReportColumnKey column : columns) {
            ReportField field = getField(JournalLineReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<JournalLineReportGroupByKey> groupBy) {
        ...
        for (JournalLineReportGroupByKey key : groupBy) {
            ReportField field = getField(JournalLineReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<JournalLineReportMetricKey> metrics) {
        ...
        for (JournalLineReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<JournalLineReportGroupByKey> groupBy, List<JournalLineReportMetricKey> metrics) {
    ...
    private void validateFilters(List<JournalLineReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<JournalLineReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (JournalLineReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<JournalLineReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (JournalLineReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(JournalLineReportFieldKey key) {
