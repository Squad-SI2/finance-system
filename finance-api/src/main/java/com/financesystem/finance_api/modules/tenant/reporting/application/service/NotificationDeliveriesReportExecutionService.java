...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.notificationdeliveries.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.NotificationDeliveriesReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.NotificationDeliveryReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.NotificationDeliveryReportRequestValidator;
...
@Service
public class NotificationDeliveriesReportExecutionService {

    private final NotificationDeliveriesReportTemplateProvider templateProvider;
    private final NotificationDeliveryReportRequestValidator validator;
    private final NotificationDeliveryReportSqlBuilder sqlBuilder;
    ...
    public NotificationDeliveriesReportExecutionService(
            NotificationDeliveriesReportTemplateProvider templateProvider,
            NotificationDeliveryReportRequestValidator validator,
            NotificationDeliveryReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationDeliveriesReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationDeliveriesReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationDeliveriesReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationDeliveriesReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<NotificationDeliveryReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationDeliveryReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<NotificationDeliveryReportGroupByKey> groupBy, List<NotificationDeliveryReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationDeliveryReportGroupByKey key : groupBy) {
            ReportField field = requireField(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (NotificationDeliveryReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<NotificationDeliveryReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        for (NotificationDeliveryReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<NotificationDeliveryReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<NotificationDeliveryReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticNotificationDeliveriesReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialNotificationDeliveriesReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticNotificationDeliveriesReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialNotificationDeliveriesReportRequest managerialRequest && managerialRequest.pagination() != null) {
    ...
    private ReportField requireField(String key) {
        ...
        if (field == null) {
            throw new ReportValidationException("Unknown notification delivery field: " + key);
        }
        return field;
    }
    ...
    private ReportMetric requireMetric(String key) {
        ...
        if (metric == null) {
            throw new ReportValidationException("Unknown notification delivery metric: " + key);
        }
        return metric;
    }
}
