...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.transactionmovements.TransactionMovementReportFilterRequest;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.query.TransactionMovementReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.TransactionMovementReportRequestValidator;
...
@Service
public class TransactionMovementsReportExecutionService {

    private final TransactionMovementsReportTemplateProvider templateProvider;
    private final TransactionMovementReportRequestValidator validator;
    private final TransactionMovementReportSqlBuilder sqlBuilder;
    ...
    public TransactionMovementsReportExecutionService(
            TransactionMovementsReportTemplateProvider templateProvider,
            TransactionMovementReportRequestValidator validator,
            TransactionMovementReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticTransactionMovementsReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticTransactionMovementsReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialTransactionMovementsReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialTransactionMovementsReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<TransactionMovementReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (TransactionMovementReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(
        ...
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (TransactionMovementReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (TransactionMovementReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<TransactionMovementReportFilterRequest> filters) {
    ...
    private List<String> resolveGroupByLabels(List<TransactionMovementReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<TransactionMovementReportMetricKey> metrics) {
    ...
    private ReportResultResponse persistAndMapResult(
        ...
        if (!artifacts.isEmpty()) {
            artifacts.forEach(artifact -> auditTrailService.recordTenantEvent(
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticTransactionMovementsReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialTransactionMovementsReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticTransactionMovementsReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialTransactionMovementsReportRequest managerialRequest && managerialRequest.pagination() != null) {
    ...
    private void validateExportPermission(List<ReportOutput> outputs) {
        ...
        if (requiresExport && !securityContextFacade.hasAuthority("reports.export")) {
            throw new org.springframework.security.access.AccessDeniedException(
                    "Missing required authority: reports.export"
            );
        }
    }
}
