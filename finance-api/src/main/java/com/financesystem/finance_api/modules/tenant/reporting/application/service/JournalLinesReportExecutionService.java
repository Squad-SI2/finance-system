...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.journallines.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.JournalLinesReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.JournalLineReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.JournalLineReportRequestValidator;
...
@Service
public class JournalLinesReportExecutionService {

    private final JournalLinesReportTemplateProvider templateProvider;
    private final JournalLineReportRequestValidator validator;
    private final JournalLineReportSqlBuilder sqlBuilder;
    ...
    public JournalLinesReportExecutionService(
            JournalLinesReportTemplateProvider templateProvider,
            JournalLineReportRequestValidator validator,
            JournalLineReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticJournalLinesReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticJournalLinesReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialJournalLinesReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialJournalLinesReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<JournalLineReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (JournalLineReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<JournalLineReportGroupByKey> groupBy, List<JournalLineReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (JournalLineReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (JournalLineReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<JournalLineReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        var template = templateProvider.getTemplate();
        for (JournalLineReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<JournalLineReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<JournalLineReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticJournalLinesReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialJournalLinesReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticJournalLinesReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialJournalLinesReportRequest managerialRequest && managerialRequest.pagination() != null) {
