...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.notificationdevices.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.NotificationDevicesReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.NotificationDeviceReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.NotificationDeviceReportRequestValidator;
...
@Service
public class NotificationDevicesReportExecutionService {

    private final NotificationDevicesReportTemplateProvider templateProvider;
    private final NotificationDeviceReportRequestValidator validator;
    private final NotificationDeviceReportSqlBuilder sqlBuilder;
    ...
    public NotificationDevicesReportExecutionService(
            NotificationDevicesReportTemplateProvider templateProvider,
            NotificationDeviceReportRequestValidator validator,
            NotificationDeviceReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationDevicesReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationDevicesReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationDevicesReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationDevicesReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<NotificationDeviceReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationDeviceReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<NotificationDeviceReportGroupByKey> groupBy, List<NotificationDeviceReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationDeviceReportGroupByKey key : groupBy) {
            ReportField field = requireField(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (NotificationDeviceReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<NotificationDeviceReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        for (NotificationDeviceReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<NotificationDeviceReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<NotificationDeviceReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticNotificationDevicesReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialNotificationDevicesReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticNotificationDevicesReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialNotificationDevicesReportRequest managerialRequest && managerialRequest.pagination() != null) {
    ...
    private ReportField requireField(String key) {
        ...
        if (field == null) {
            throw new ReportValidationException("Unknown notification device field: " + key);
        }
        return field;
    }
    ...
    private ReportMetric requireMetric(String key) {
        ...
        if (metric == null) {
            throw new ReportValidationException("Unknown notification device metric: " + key);
        }
        return metric;
    }
}
