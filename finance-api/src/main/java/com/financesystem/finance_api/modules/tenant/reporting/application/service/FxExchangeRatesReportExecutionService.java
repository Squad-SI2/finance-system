...
import com.financesystem.finance_api.modules.tenant.reporting.application.query.FxExchangeRateReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.FxExchangeRateReportRequestValidator;
...
@Service
public class FxExchangeRatesReportExecutionService {
    private final FxExchangeRatesReportTemplateProvider templateProvider;
    private final FxExchangeRateReportRequestValidator validator;
    private final FxExchangeRateReportSqlBuilder sqlBuilder;
    ...
    public FxExchangeRatesReportExecutionService(FxExchangeRatesReportTemplateProvider templateProvider, FxExchangeRateReportRequestValidator validator, FxExchangeRateReportSqlBuilder sqlBuilder, ReportJdbcExecutor jdbcExecutor, ReportHeaderBuilder headerBuilder, ReportExportService exportService, AuditTrailService auditTrailService, ReportExecutionRepository reportExecutionRepository, ObjectMapper objectMapper, SecurityContextFacade securityContextFacade) {
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticFxExchangeRateReportRequest request) { return executeAnalytic(request, null); }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticFxExchangeRateReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialFxExchangeRateReportRequest request) { return executeManagerial(request, null); }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialFxExchangeRateReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<FxExchangeRateReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (FxExchangeRateReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<FxExchangeRateReportGroupByKey> groupBy, List<FxExchangeRateReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (FxExchangeRateReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (FxExchangeRateReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<FxExchangeRateReportFilterRequest> filters) {
    ...
    private List<String> resolveGroupByLabels(List<FxExchangeRateReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<FxExchangeRateReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticFxExchangeRateReportRequest analyticRequest) return analyticRequest.outputs();
        if (request instanceof RunManagerialFxExchangeRateReportRequest managerialRequest) return managerialRequest.outputs();
        return List.of();
    }

    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticFxExchangeRateReportRequest analyticRequest && analyticRequest.pagination() != null) return analyticRequest.pagination().limit();
        if (request instanceof RunManagerialFxExchangeRateReportRequest managerialRequest && managerialRequest.pagination() != null) return managerialRequest.pagination().limit();
        return null;
    }

    private Integer extractOffset(Object request) {
        if (request instanceof RunAnalyticFxExchangeRateReportRequest analyticRequest && analyticRequest.pagination() != null) return analyticRequest.pagination().offset();
        if (request instanceof RunManagerialFxExchangeRateReportRequest managerialRequest && managerialRequest.pagination() != null) return managerialRequest.pagination().offset();
        return null;
    }
