...
@Service
public class ListTenantBackupsUseCase {

    private final BackupApplicationService backupApplicationService;
    private final BackupJobMapper backupJobMapper;

    public ListTenantBackupsUseCase(BackupApplicationService backupApplicationService, BackupJobMapper backupJobMapper) {
    ...
    public List<BackupJobResponse> execute() {
        return backupApplicationService.listTenantBackups().stream().map(backupJobMapper::toResponse).toList();
    }
}
