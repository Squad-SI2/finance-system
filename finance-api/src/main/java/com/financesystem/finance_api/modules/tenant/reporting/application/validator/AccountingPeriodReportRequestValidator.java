...
import com.financesystem.finance_api.modules.tenant.reporting.domain.model.accountingperiods.*;
...
@Component
public class AccountingPeriodReportRequestValidator {

    private final ReportTemplate template;

    public AccountingPeriodReportRequestValidator(AccountingPeriodsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticAccountingPeriodsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.ACCOUNTING_PERIODS);
    ...
    public void validateManagerial(RunManagerialAccountingPeriodsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.ACCOUNTING_PERIODS);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<AccountingPeriodReportColumnKey> columns) {
        ...
        for (AccountingPeriodReportColumnKey column : columns) {
            ReportField field = getField(AccountingPeriodReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<AccountingPeriodReportGroupByKey> groupBy) {
        ...
        for (AccountingPeriodReportGroupByKey key : groupBy) {
            ReportField field = getField(AccountingPeriodReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<AccountingPeriodReportMetricKey> metrics) {
        ...
        for (AccountingPeriodReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<AccountingPeriodReportGroupByKey> groupBy, List<AccountingPeriodReportMetricKey> metrics) {
    ...
    private void validateFilters(List<AccountingPeriodReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<AccountingPeriodReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (AccountingPeriodReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<AccountingPeriodReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (AccountingPeriodReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(AccountingPeriodReportFieldKey key) {
