...
@Component
public class QrTransactionIntentReportRequestValidator {

    private final ReportTemplate template;

    public QrTransactionIntentReportRequestValidator(QrTransactionIntentsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticQrTransactionIntentReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.QR_TRANSACTION_INTENTS);
    ...
    public void validateManagerial(RunManagerialQrTransactionIntentReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.QR_TRANSACTION_INTENTS);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<QrTransactionIntentReportColumnKey> columns) {
        ...
        for (QrTransactionIntentReportColumnKey column : columns) {
            ReportField field = getField(QrTransactionIntentReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<QrTransactionIntentReportGroupByKey> groupBy) {
        ...
        for (QrTransactionIntentReportGroupByKey key : groupBy) {
            ReportField field = getField(QrTransactionIntentReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<QrTransactionIntentReportMetricKey> metrics) {
        ...
        for (QrTransactionIntentReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<QrTransactionIntentReportGroupByKey> groupBy, List<QrTransactionIntentReportMetricKey> metrics) {
    ...
    private void validateFilters(List<QrTransactionIntentReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<QrTransactionIntentReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (QrTransactionIntentReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<QrTransactionIntentReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (QrTransactionIntentReportManagerialSortRequest item : sort) {
    ...
    private void validateNumericValue(JsonNode value, JsonNode to, ReportOperator operator) {
        ...
        if (operator == ReportOperator.BETWEEN) {
            if (value == null || to == null) {
                throw new ReportValidationException("BETWEEN numeric filter requires value and to");
            }
            return;
        }
        if (value == null || (!value.isNumber() && !value.isTextual())) {
            throw new ReportValidationException("Numeric filter requires a numeric value");
        }
    }
    ...
    private void validateEnumValue(ReportField field, ReportOperator operator, JsonNode value, JsonNode to) {
        ...
        if (value == null || !value.isTextual() || !StringUtils.hasText(value.asText())) {
            throw new ReportValidationException("Enum filter requires a non-empty string");
        }
    }
    ...
    private ReportField getField(QrTransactionIntentReportFieldKey key) {
