...
@Component
public class NotificationReportSqlBuilder {

    private final ReportTemplate template;

    public NotificationReportSqlBuilder(NotificationsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public ReportSqlQuery buildAnalytic(RunAnalyticNotificationsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        List<NotificationReportColumnKey> columns = request.columns();
    ...
    public ReportSqlQuery buildManagerial(RunManagerialNotificationsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        List<NotificationReportGroupByKey> groupBy = request.groupBy();
        List<NotificationReportMetricKey> metrics = request.metrics();
    ...
    private void appendWhere(StringBuilder sql, List<Object> parameters, List<NotificationReportFilterRequest> filters) {
    ...
    private void appendGroupBy(StringBuilder sql, List<NotificationReportGroupByKey> groupBy, Map<String, ReportField> fieldsByKey) {
        sql.append(" GROUP BY ");
    ...
    private void appendOrderBy(StringBuilder sql, Map<String, ReportField> fieldsByKey, List<?> sortItems) {
        ...
        for (Object sortItem : sortItems) {
            if (sortItem instanceof NotificationReportSortRequest analyticSort) {
            ...
            } else if (sortItem instanceof NotificationReportManagerialSortRequest managerialSort) {
    ...
    private String metricSelectExpression(NotificationReportMetricKey metricKey) {
