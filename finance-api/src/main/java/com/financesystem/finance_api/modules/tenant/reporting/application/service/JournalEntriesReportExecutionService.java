...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.journalentries.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.JournalEntriesReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.JournalEntryReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.JournalEntryReportRequestValidator;
...
@Service
public class JournalEntriesReportExecutionService {

    private final JournalEntriesReportTemplateProvider templateProvider;
    private final JournalEntryReportRequestValidator validator;
    private final JournalEntryReportSqlBuilder sqlBuilder;
    ...
    public JournalEntriesReportExecutionService(
            JournalEntriesReportTemplateProvider templateProvider,
            JournalEntryReportRequestValidator validator,
            JournalEntryReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticJournalEntriesReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticJournalEntriesReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialJournalEntriesReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialJournalEntriesReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<JournalEntryReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (JournalEntryReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<JournalEntryReportGroupByKey> groupBy, List<JournalEntryReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (JournalEntryReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (JournalEntryReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<JournalEntryReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        var template = templateProvider.getTemplate();
        for (JournalEntryReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<JournalEntryReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<JournalEntryReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticJournalEntriesReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialJournalEntriesReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticJournalEntriesReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialJournalEntriesReportRequest managerialRequest && managerialRequest.pagination() != null) {
