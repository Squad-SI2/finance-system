...
@Component
public class NotificationDeviceReportRequestValidator {

    private final ReportTemplate template;

    public NotificationDeviceReportRequestValidator(NotificationDevicesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticNotificationDevicesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATION_DEVICES);
    ...
    public void validateManagerial(RunManagerialNotificationDevicesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATION_DEVICES);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<NotificationDeviceReportColumnKey> columns) {
        ...
        for (NotificationDeviceReportColumnKey column : columns) {
            ReportField field = getField(NotificationDeviceReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<NotificationDeviceReportGroupByKey> groupBy) {
        ...
        for (NotificationDeviceReportGroupByKey key : groupBy) {
            ReportField field = getField(NotificationDeviceReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<NotificationDeviceReportMetricKey> metrics) {
        ...
        for (NotificationDeviceReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<NotificationDeviceReportGroupByKey> groupBy, List<NotificationDeviceReportMetricKey> metrics) {
    ...
    private void validateFilters(List<NotificationDeviceReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<NotificationDeviceReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationDeviceReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<NotificationDeviceReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationDeviceReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(NotificationDeviceReportFieldKey key) {
