...
import com.financesystem.finance_api.modules.tenant.reporting.application.query.OperationFeeReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.OperationFeeReportRequestValidator;
...
@Service
public class OperationFeesReportExecutionService {
    private final OperationFeesReportTemplateProvider templateProvider;
    private final OperationFeeReportRequestValidator validator;
    private final OperationFeeReportSqlBuilder sqlBuilder;
    ...
    public OperationFeesReportExecutionService(OperationFeesReportTemplateProvider templateProvider, OperationFeeReportRequestValidator validator, OperationFeeReportSqlBuilder sqlBuilder, ReportJdbcExecutor jdbcExecutor, ReportHeaderBuilder headerBuilder, ReportExportService exportService, AuditTrailService auditTrailService, ReportExecutionRepository reportExecutionRepository, ObjectMapper objectMapper, SecurityContextFacade securityContextFacade) {
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticOperationFeeReportRequest request) { return executeAnalytic(request, null); }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticOperationFeeReportRequest request, UUID sourceExecutionId) {
        ...
        try {
            ...
            ReportHeaderResponse header = headerBuilder.build(template.title(), ReportMode.ANALYTIC, generatedAt, appliedFilters, columns.stream().map(ReportColumnResponse::label).toList(), List.of(), List.of());
            List<ReportExportArtifact> artifacts = exportService.export(template.reportType(), ReportMode.ANALYTIC, generatedAt, request.outputs(), template.exportOptions().pdf() == null ? 0 : template.exportOptions().pdf().maxColumns(), header, columns, rows);
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialOperationFeeReportRequest request) { return executeManagerial(request, null); }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialOperationFeeReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
            ...
            List<String> appliedFilters = summarizeFilters(request.filters());
            ReportHeaderResponse header = headerBuilder.build(template.title(), ReportMode.MANAGERIAL, generatedAt, appliedFilters, List.of(), selectedMetrics, selectedGroupBy);
            List<ReportExportArtifact> artifacts = exportService.export(template.reportType(), ReportMode.MANAGERIAL, generatedAt, request.outputs(), template.exportOptions().pdf() == null ? 0 : template.exportOptions().pdf().maxColumns(), header, columns, rows);
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<OperationFeeReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (OperationFeeReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<OperationFeeReportGroupByKey> groupBy, List<OperationFeeReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (OperationFeeReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (OperationFeeReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<OperationFeeReportFilterRequest> filters) {
        ...
        for (OperationFeeReportFilterRequest filter : filters) {
            ...
            descriptions.add(headerBuilder.describeFilter(field.label(), filter.operator(), filter.value(), filter.to()));
        }
        return descriptions;
    }

    private List<String> resolveGroupByLabels(List<OperationFeeReportGroupByKey> groupBy) {
        if (groupBy == null || groupBy.isEmpty()) return List.of();
        var template = templateProvider.getTemplate();
    ...
    private List<String> resolveMetricLabels(List<OperationFeeReportMetricKey> metrics) {
    ...
    private ReportResultResponse persistAndMapResult(ReportTemplate template, ReportMode mode, Object request, Instant generatedAt, UUID sourceExecutionId, ReportHeaderResponse header, List<ReportColumnResponse> columns, List<Map<String, Object>> rows, List<ReportExportArtifact> artifacts) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticOperationFeeReportRequest analyticRequest) return analyticRequest.outputs();
        if (request instanceof RunManagerialOperationFeeReportRequest managerialRequest) return managerialRequest.outputs();
        return List.of();
    }

    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticOperationFeeReportRequest analyticRequest && analyticRequest.pagination() != null) return analyticRequest.pagination().limit();
        if (request instanceof RunManagerialOperationFeeReportRequest managerialRequest && managerialRequest.pagination() != null) return managerialRequest.pagination().limit();
        return null;
    }

    private Integer extractOffset(Object request) {
        if (request instanceof RunAnalyticOperationFeeReportRequest analyticRequest && analyticRequest.pagination() != null) return analyticRequest.pagination().offset();
        if (request instanceof RunManagerialOperationFeeReportRequest managerialRequest && managerialRequest.pagination() != null) return managerialRequest.pagination().offset();
        return null;
    }

    private String serialize(Object value) {
        try { return objectMapper.writeValueAsString(value); } catch (Exception ex) { throw new ReportException("Failed to serialize report payload", ex); }
    }

    private String safeErrorMessage(RuntimeException ex) {
        if (ex == null || ex.getMessage() == null || ex.getMessage().isBlank()) return ex == null ? "Unknown report execution error" : ex.getClass().getSimpleName();
    ...
    private void validateExportPermission(List<ReportOutput> outputs) {
        ...
        if (requiresExport && !securityContextFacade.hasAuthority("reports.export")) {
            throw new org.springframework.security.access.AccessDeniedException("Missing required authority: reports.export");
        }
    }
}
