...
@Component
public class AccountsReportExecutionHandler implements ReportExecutionHandler {

    private final ReportExecutionService reportExecutionService;
    private final ObjectMapper objectMapper;

    public AccountsReportExecutionHandler(
    ...
    private RunAnalyticAccountReportRequest deserializeAnalytic(JsonNode requestBody) {
        try {
            return objectMapper.treeToValue(requestBody, RunAnalyticAccountReportRequest.class);
        } catch (Exception ex) {
            throw new ReportExecutionPayloadException("Failed to deserialize analytic accounts report request", ex);
        }
    }

    private RunManagerialAccountReportRequest deserializeManagerial(JsonNode requestBody) {
        try {
            return objectMapper.treeToValue(requestBody, RunManagerialAccountReportRequest.class);
        } catch (Exception ex) {
            throw new ReportExecutionPayloadException("Failed to deserialize managerial accounts report request", ex);
        }
    }

    private RunAnalyticAccountReportRequest withOutputs(RunAnalyticAccountReportRequest request, List<ReportOutput> outputs) {
        return new RunAnalyticAccountReportRequest(
                request.reportType(),
                request.columns(),
                request.filters(),
                request.sort(),
    ...
    private RunManagerialAccountReportRequest withOutputs(RunManagerialAccountReportRequest request, List<ReportOutput> outputs) {
        return new RunManagerialAccountReportRequest(
                request.reportType(),
                request.groupBy(),
                request.metrics(),
                request.filters(),
