...
@Component
public class NotificationReportRequestValidator {

    private final ReportTemplate template;

    public NotificationReportRequestValidator(NotificationsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticNotificationsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATIONS);
    ...
    public void validateManagerial(RunManagerialNotificationsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATIONS);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<NotificationReportColumnKey> columns) {
        ...
        for (NotificationReportColumnKey column : columns) {
            ReportField field = getField(NotificationReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<NotificationReportGroupByKey> groupBy) {
        ...
        for (NotificationReportGroupByKey key : groupBy) {
            ReportField field = getField(NotificationReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<NotificationReportMetricKey> metrics) {
        ...
        for (NotificationReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<NotificationReportGroupByKey> groupBy, List<NotificationReportMetricKey> metrics) {
    ...
    private void validateFilters(List<NotificationReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<NotificationReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<NotificationReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(NotificationReportFieldKey key) {
