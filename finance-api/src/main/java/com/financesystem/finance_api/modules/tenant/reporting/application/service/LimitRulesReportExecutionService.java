...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.limitrules.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.LimitRulesReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.LimitRuleReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.LimitRuleReportRequestValidator;
...
@Service
public class LimitRulesReportExecutionService {

    private final LimitRulesReportTemplateProvider templateProvider;
    private final LimitRuleReportRequestValidator validator;
    private final LimitRuleReportSqlBuilder sqlBuilder;
    ...
    public LimitRulesReportExecutionService(
            LimitRulesReportTemplateProvider templateProvider,
            LimitRuleReportRequestValidator validator,
            LimitRuleReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticLimitRulesReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticLimitRulesReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialLimitRulesReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialLimitRulesReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<LimitRuleReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (LimitRuleReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(
        ...
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (LimitRuleReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (LimitRuleReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<LimitRuleReportFilterRequest> filters) {
        ...
        for (LimitRuleReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<LimitRuleReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<LimitRuleReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticLimitRulesReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialLimitRulesReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticLimitRulesReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialLimitRulesReportRequest managerialRequest && managerialRequest.pagination() != null) {
