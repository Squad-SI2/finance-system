...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.notifications.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.NotificationsReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.NotificationReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.NotificationReportRequestValidator;
...
@Service
public class NotificationsReportExecutionService {

    private final NotificationsReportTemplateProvider templateProvider;
    private final NotificationReportRequestValidator validator;
    private final NotificationReportSqlBuilder sqlBuilder;
    ...
    public NotificationsReportExecutionService(
            NotificationsReportTemplateProvider templateProvider,
            NotificationReportRequestValidator validator,
            NotificationReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationsReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticNotificationsReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationsReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialNotificationsReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<NotificationReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<NotificationReportGroupByKey> groupBy, List<NotificationReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (NotificationReportGroupByKey key : groupBy) {
            ReportField field = requireField(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (NotificationReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<NotificationReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }

        List<String> descriptions = new ArrayList<>();
        for (NotificationReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<NotificationReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<NotificationReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticNotificationsReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialNotificationsReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticNotificationsReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialNotificationsReportRequest managerialRequest && managerialRequest.pagination() != null) {
    ...
    private ReportField requireField(String key) {
        ...
        if (field == null) {
            throw new ReportValidationException("Unknown notification field: " + key);
        }
        return field;
    }
    ...
    private ReportMetric requireMetric(String key) {
        ...
        if (metric == null) {
            throw new ReportValidationException("Unknown notification metric: " + key);
        }
        return metric;
    }
}
