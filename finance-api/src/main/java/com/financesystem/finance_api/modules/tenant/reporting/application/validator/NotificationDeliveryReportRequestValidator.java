...
@Component
public class NotificationDeliveryReportRequestValidator {

    private final ReportTemplate template;

    public NotificationDeliveryReportRequestValidator(NotificationDeliveriesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticNotificationDeliveriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATION_DELIVERIES);
    ...
    public void validateManagerial(RunManagerialNotificationDeliveriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.NOTIFICATION_DELIVERIES);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<NotificationDeliveryReportColumnKey> columns) {
        ...
        for (NotificationDeliveryReportColumnKey column : columns) {
            ReportField field = getField(NotificationDeliveryReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<NotificationDeliveryReportGroupByKey> groupBy) {
        ...
        for (NotificationDeliveryReportGroupByKey key : groupBy) {
            ReportField field = getField(NotificationDeliveryReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<NotificationDeliveryReportMetricKey> metrics) {
        ...
        for (NotificationDeliveryReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<NotificationDeliveryReportGroupByKey> groupBy, List<NotificationDeliveryReportMetricKey> metrics) {
    ...
    private void validateFilters(List<NotificationDeliveryReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<NotificationDeliveryReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationDeliveryReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<NotificationDeliveryReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (NotificationDeliveryReportManagerialSortRequest item : sort) {
    ...
    private void validateFilterValue(ReportField field, ReportOperator operator, JsonNode value, JsonNode to) {
        ...
        if (field.type() == ReportFieldType.ENUM) {
            validateEnumValue(value, operator);
            return;
        }
    ...
    private void validateEnumValue(JsonNode value, ReportOperator operator) {
        if (operator == ReportOperator.IN) {
            if (value == null || !value.isArray() || value.isEmpty()) {
    ...
    private ReportField getField(NotificationDeliveryReportFieldKey key) {
