...
@Component
public class JournalEntryReportRequestValidator {

    private final ReportTemplate template;

    public JournalEntryReportRequestValidator(JournalEntriesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticJournalEntriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.JOURNAL_ENTRIES);
    ...
    public void validateManagerial(RunManagerialJournalEntriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.JOURNAL_ENTRIES);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<JournalEntryReportColumnKey> columns) {
        ...
        for (JournalEntryReportColumnKey column : columns) {
            ReportField field = getField(JournalEntryReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<JournalEntryReportGroupByKey> groupBy) {
        ...
        for (JournalEntryReportGroupByKey key : groupBy) {
            ReportField field = getField(JournalEntryReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<JournalEntryReportMetricKey> metrics) {
        ...
        for (JournalEntryReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<JournalEntryReportGroupByKey> groupBy, List<JournalEntryReportMetricKey> metrics) {
    ...
    private void validateFilters(List<JournalEntryReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<JournalEntryReportSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (JournalEntryReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<JournalEntryReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }
        for (JournalEntryReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(JournalEntryReportFieldKey key) {
