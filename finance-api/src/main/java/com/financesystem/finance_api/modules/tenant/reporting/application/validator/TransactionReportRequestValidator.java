...
import com.financesystem.finance_api.modules.tenant.reporting.domain.model.transactions.*;
...
@Component
public class TransactionReportRequestValidator {

    private final ReportTemplate template;

    public TransactionReportRequestValidator(TransactionsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticTransactionReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.TRANSACTIONS);
    ...
    public void validateManagerial(RunManagerialTransactionReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.TRANSACTIONS);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<TransactionReportColumnKey> columns) {
        ...
        for (TransactionReportColumnKey column : columns) {
            TransactionReportFieldKey fieldKey = TransactionReportFieldKey.valueOf(column.name());
    ...
    private void validateGroupBy(List<TransactionReportGroupByKey> groupBy) {
        ...
        for (TransactionReportGroupByKey key : groupBy) {
            ReportField field = getField(TransactionReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<TransactionReportMetricKey> metrics) {
        ...
        for (TransactionReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<TransactionReportGroupByKey> groupBy, List<TransactionReportMetricKey> metrics) {
    ...
    private void validateFilters(List<TransactionReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<TransactionReportSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (TransactionReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<TransactionReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (TransactionReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(TransactionReportFieldKey key) {
