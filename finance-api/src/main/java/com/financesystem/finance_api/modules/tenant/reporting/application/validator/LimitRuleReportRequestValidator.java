...
import com.financesystem.finance_api.modules.tenant.reporting.domain.model.limitrules.*;
...
@Component
public class LimitRuleReportRequestValidator {

    private final ReportTemplate template;

    public LimitRuleReportRequestValidator(LimitRulesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticLimitRulesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.LIMIT_RULES);
    ...
    public void validateManagerial(RunManagerialLimitRulesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.LIMIT_RULES);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<LimitRuleReportColumnKey> columns) {
        ...
        for (LimitRuleReportColumnKey column : columns) {
            ReportField field = getField(LimitRuleReportFieldKey.valueOf(column.name()));
            if (!field.columnable()) {
    ...
    private void validateGroupBy(List<LimitRuleReportGroupByKey> groupBy) {
        ...
        for (LimitRuleReportGroupByKey key : groupBy) {
            ReportField field = getField(LimitRuleReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<LimitRuleReportMetricKey> metrics) {
        ...
        for (LimitRuleReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<LimitRuleReportGroupByKey> groupBy, List<LimitRuleReportMetricKey> metrics) {
    ...
    private void validateFilters(List<LimitRuleReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<LimitRuleReportSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (LimitRuleReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<LimitRuleReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (LimitRuleReportManagerialSortRequest item : sort) {
    ...
    private ReportField getField(LimitRuleReportFieldKey key) {
