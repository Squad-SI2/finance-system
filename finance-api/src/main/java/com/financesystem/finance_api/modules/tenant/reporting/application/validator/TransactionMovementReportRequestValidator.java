...
@Component
public class TransactionMovementReportRequestValidator {

    private final ReportTemplate template;

    public TransactionMovementReportRequestValidator(TransactionMovementsReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public void validateAnalytic(RunAnalyticTransactionMovementsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.TRANSACTION_MOVEMENTS);
    ...
    public void validateManagerial(RunManagerialTransactionMovementsReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        requireReportType(request.reportType(), ReportType.TRANSACTION_MOVEMENTS);
        validateOutputs(request.outputs(), Set.of(ReportOutput.SCREEN, ReportOutput.PDF, ReportOutput.XLSX));
        validatePagination(request.pagination());
    ...
    private void validateColumns(List<TransactionMovementReportColumnKey> columns) {
        ...
        for (TransactionMovementReportColumnKey column : columns) {
            TransactionMovementReportFieldKey fieldKey = TransactionMovementReportFieldKey.valueOf(column.name());
    ...
    private void validateGroupBy(List<TransactionMovementReportGroupByKey> groupBy) {
        ...
        for (TransactionMovementReportGroupByKey key : groupBy) {
            ReportField field = getField(TransactionMovementReportFieldKey.valueOf(key.name()));
            if (!field.groupable()) {
    ...
    private void validateMetrics(List<TransactionMovementReportMetricKey> metrics) {
        ...
        for (TransactionMovementReportMetricKey metricKey : metrics) {
    ...
    private void validateMaxResultColumns(List<TransactionMovementReportGroupByKey> groupBy, List<TransactionMovementReportMetricKey> metrics) {
    ...
    private void validateFilters(List<TransactionMovementReportFilterRequest> filters) {
        if (filters == null) {
            return;
        }
    ...
    private void validateAnalyticSort(List<TransactionMovementReportSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (TransactionMovementReportSortRequest item : sort) {
    ...
    private void validateManagerialSort(List<TransactionMovementReportManagerialSortRequest> sort) {
        if (sort == null) {
            return;
        }

        for (TransactionMovementReportManagerialSortRequest item : sort) {
