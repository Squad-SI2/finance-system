...
@Component
public class NotificationDeliveryReportSqlBuilder {

    private final ReportTemplate template;

    public NotificationDeliveryReportSqlBuilder(NotificationDeliveriesReportTemplateProvider provider) {
        this.template = provider.getTemplate();
    }

    public ReportSqlQuery buildAnalytic(RunAnalyticNotificationDeliveriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        List<NotificationDeliveryReportColumnKey> columns = request.columns();
        ...
        sql.append(" LEFT JOIN tenant_notifications n ON n.id = d.notification_id");
    ...
    public ReportSqlQuery buildManagerial(RunManagerialNotificationDeliveriesReportRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        List<NotificationDeliveryReportGroupByKey> groupBy = request.groupBy();
        List<NotificationDeliveryReportMetricKey> metrics = request.metrics();
    ...
    private void appendWhere(StringBuilder sql, List<Object> parameters, List<NotificationDeliveryReportFilterRequest> filters) {
    ...
    private void appendGroupBy(StringBuilder sql, List<NotificationDeliveryReportGroupByKey> groupBy, Map<String, ReportField> fieldsByKey) {
        sql.append(" GROUP BY ");
    ...
    private void appendOrderBy(StringBuilder sql, Map<String, ReportField> fieldsByKey, List<?> sortItems) {
        ...
        for (Object sortItem : sortItems) {
            if (sortItem instanceof NotificationDeliveryReportSortRequest analyticSort) {
            ...
            } else if (sortItem instanceof NotificationDeliveryReportManagerialSortRequest managerialSort) {
    ...
    private Object convertValue(ReportField field, JsonNode node) {
        ...
        return switch (field.type()) {
            ...
            case UUID -> UUID.fromString(requireText(node));
            ...
            case DATE -> requireDateText(node);
        };
    }
    ...
    private Timestamp convertDatetime(JsonNode node) {
    ...
    private String metricSelectExpression(NotificationDeliveryReportMetricKey metricKey) {
