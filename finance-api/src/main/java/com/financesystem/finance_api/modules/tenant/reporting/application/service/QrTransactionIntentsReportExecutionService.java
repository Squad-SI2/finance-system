...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.qrtransactionintents.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.QrTransactionIntentsReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.QrTransactionIntentReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.QrTransactionIntentReportRequestValidator;
...
@Service
public class QrTransactionIntentsReportExecutionService {

    private final QrTransactionIntentsReportTemplateProvider templateProvider;
    private final QrTransactionIntentReportRequestValidator validator;
    private final QrTransactionIntentReportSqlBuilder sqlBuilder;
    ...
    public QrTransactionIntentsReportExecutionService(
            QrTransactionIntentsReportTemplateProvider templateProvider,
            QrTransactionIntentReportRequestValidator validator,
            QrTransactionIntentReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticQrTransactionIntentReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticQrTransactionIntentReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialQrTransactionIntentReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialQrTransactionIntentReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<QrTransactionIntentReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (QrTransactionIntentReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<com.financesystem.finance_api.modules.tenant.reporting.domain.model.qrtransactionintents.QrTransactionIntentReportGroupByKey> groupBy, List<com.financesystem.finance_api.modules.tenant.reporting.domain.model.qrtransactionintents.QrTransactionIntentReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (var key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
    ...
    private List<String> summarizeFilters(List<QrTransactionIntentReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        var template = templateProvider.getTemplate();
        for (QrTransactionIntentReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<com.financesystem.finance_api.modules.tenant.reporting.domain.model.qrtransactionintents.QrTransactionIntentReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<com.financesystem.finance_api.modules.tenant.reporting.domain.model.qrtransactionintents.QrTransactionIntentReportMetricKey> metrics) {
    ...
    private ReportResultResponse persistAndMapResult(
        ...
        ReportExecution savedExecution = reportExecutionRepository.save(
                new ReportExecution(
                        null,
        ...
        auditTrailService.recordTenantEvent(
                ...
                "report",
                template.reportType().name(),
                Map.of("mode", mode.name(), "outputs", extractOutputs(request), "rowCount", rows.size())
        );
        ...
        return new ReportResultResponse(
                ...
                new ReportResultMetadataResponse(rows.size(), extractLimit(request), extractOffset(request))
        );
    }
        ...
        try {
            ...
            reportExecutionRepository.save(
                    new ReportExecution(
                            ...
                            template.reportType().name(),
                            template.title(),
                            template.title() + " - " + mode.name() + " - " + generatedAt,
                            mode.name(),
                            securityContextFacade.getCurrentSubject(),
                            serialize(request),
                            serialize(extractOutputs(request)),
                            0,
                            sourceExecutionId,
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticQrTransactionIntentReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialQrTransactionIntentReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticQrTransactionIntentReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialQrTransactionIntentReportRequest managerialRequest && managerialRequest.pagination() != null) {
