...
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.ReportResultMetadataResponse;
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.ReportResultResponse;
import com.financesystem.finance_api.modules.tenant.reporting.application.dto.accountingperiods.*;
...
import com.financesystem.finance_api.modules.tenant.reporting.application.provider.AccountingPeriodsReportTemplateProvider;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.AccountingPeriodReportSqlBuilder;
import com.financesystem.finance_api.modules.tenant.reporting.application.query.ReportSqlQuery;
import com.financesystem.finance_api.modules.tenant.reporting.application.validator.AccountingPeriodReportRequestValidator;
...
@Service
public class AccountingPeriodsReportExecutionService {

    private final AccountingPeriodsReportTemplateProvider templateProvider;
    private final AccountingPeriodReportRequestValidator validator;
    private final AccountingPeriodReportSqlBuilder sqlBuilder;
    ...
    public AccountingPeriodsReportExecutionService(
            AccountingPeriodsReportTemplateProvider templateProvider,
            AccountingPeriodReportRequestValidator validator,
            AccountingPeriodReportSqlBuilder sqlBuilder,
    ...
    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticAccountingPeriodsReportRequest request) {
        return executeAnalytic(request, null);
    }

    @Transactional
    public ReportResultResponse executeAnalytic(RunAnalyticAccountingPeriodsReportRequest request, UUID sourceExecutionId) {
    ...
    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialAccountingPeriodsReportRequest request) {
        return executeManagerial(request, null);
    }

    @Transactional
    public ReportResultResponse executeManagerial(RunManagerialAccountingPeriodsReportRequest request, UUID sourceExecutionId) {
        Instant generatedAt = Instant.now();
        validateExportPermission(request.outputs());
        try {
    ...
    private List<ReportColumnResponse> resolveAnalyticColumns(List<AccountingPeriodReportColumnKey> requestedColumns) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (AccountingPeriodReportColumnKey key : requestedColumns) {
    ...
    private List<ReportColumnResponse> resolveManagerialColumns(List<AccountingPeriodReportGroupByKey> groupBy, List<AccountingPeriodReportMetricKey> metrics) {
        List<ReportColumnResponse> columns = new ArrayList<>();
        for (AccountingPeriodReportGroupByKey key : groupBy) {
            ReportField field = templateProvider.getTemplate().fields().get(key.name());
            columns.add(new ReportColumnResponse(key, field.label(), field.type()));
        }
        for (AccountingPeriodReportMetricKey key : metrics) {
    ...
    private List<String> summarizeFilters(List<AccountingPeriodReportFilterRequest> filters) {
        if (filters == null || filters.isEmpty()) {
            return List.of();
        }
        List<String> descriptions = new ArrayList<>();
        var template = templateProvider.getTemplate();
        for (AccountingPeriodReportFilterRequest filter : filters) {
    ...
    private List<String> resolveGroupByLabels(List<AccountingPeriodReportGroupByKey> groupBy) {
    ...
    private List<String> resolveMetricLabels(List<AccountingPeriodReportMetricKey> metrics) {
    ...
    private List<ReportOutput> extractOutputs(Object request) {
        if (request instanceof RunAnalyticAccountingPeriodsReportRequest analyticRequest) {
            return analyticRequest.outputs();
        }
        if (request instanceof RunManagerialAccountingPeriodsReportRequest managerialRequest) {
    ...
    private Integer extractLimit(Object request) {
        if (request instanceof RunAnalyticAccountingPeriodsReportRequest analyticRequest && analyticRequest.pagination() != null) {
            return analyticRequest.pagination().limit();
        }
        if (request instanceof RunManagerialAccountingPeriodsReportRequest managerialRequest && managerialRequest.pagination() != null) {
