...
import '../viewmodels/activate_account_viewmodel.dart';

class ActivateAccountPage extends StatefulWidget {
  final String? initialTenant;
  final String? token;

  const ActivateAccountPage({super.key, this.initialTenant, this.token});
...
class _ActivateAccountPageState extends State<ActivateAccountPage> {
  ...
  late ActivateAccountViewModel _viewModel;

  @override
  void initState() {
    super.initState();
    _viewModel = di.sl<ActivateAccountViewModel>();
  ...
  Future<void> _activate() async {
    if (_formKey.currentState!.validate()) {
      final success = await _viewModel.activateAccount(
        tenantController.text.trim(),
        tokenController.text.trim(),
      );
      if (success && mounted) {
        _showSnackBar('Cuenta activada. Inicia sesión.', isError: false);
