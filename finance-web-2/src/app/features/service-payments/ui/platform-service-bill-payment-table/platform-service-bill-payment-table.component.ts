...
@Component({
  selector: 'app-platform-service-bill-payment-table',
  standalone: true,
        ...
        <table class="min-w-full divide-y divide-[#E8F2E2]">
          <thead class="bg-[#FAFCF8]">
            <tr>
              ...
              <th class="px-5 py-3 text-left text-xs font-bold uppercase tracking-wide text-[#567157]">Tenant</th>
          ...
          <tbody class="divide-y divide-[#E8F2E2] bg-white">
            <tr *ngFor="let payment of payments" class="hover:bg-[#FAFCF8]">
              ...
              <td class="whitespace-nowrap px-5 py-4 text-sm text-[#6B7D6C]">{{ payment.paidByTenantSlug || '-' }}</td>
              <td class="whitespace-nowrap px-5 py-4 text-sm text-[#6B7D6C]">{{ payment.paidByAccountNumber || '-' }}</td>
            ...
            <tr *ngIf="payments.length === 0">
              <td colspan="8" class="px-5 py-10 text-center">
                <p class="text-sm font-semibold text-[#1B5E20]">No hay pagos globales registrados</p>
                <p class="mt-1 text-sm text-[#6B7D6C]">Los pagos aparecerán aquí cuando clientes o ventanilla paguen servicios.</p>
              </td>
            </tr>
          </tbody>
        </table>
