...
@Component({
  ...
  template: `
    ...
    <div *ngIf="balanceData" class="fixed inset-0 z-[100] flex items-start justify-center p-4 sm:items-center">
      ...
      <!-- Contenido del modal -->
      <div class="relative w-full max-w-sm max-h-[calc(100vh-2rem)] overflow-y-auto overscroll-contain bg-card rounded-2xl shadow-2xl border border-border p-6 transform transition-all">
        <!-- Cerrar -->
        <button 
          (click)="closeBalanceModal()"
